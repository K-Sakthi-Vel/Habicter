...
module.exports.daysCount=async function(req,res){
	...
	const updateCompletionDays = async() => {
	...
	if(habitObj.createdAt.getMilliseconds() === habitObj.updatedAt.getMilliseconds()){

		updateCompletionDays();
	}
	else if(currentDate > habitObj.updatedAt.getDate()){

		updateCompletionDays();

	}
	else if((habitObj.updatedAt.getDate() in [29,30,31]) && currentDate >= "01"){

		updateCompletionDays();

	}else{
		console.log("Hooray!!! Habit considered today, see you tomorrow")
		return res.redirect("/")
